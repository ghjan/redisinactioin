...
#         conn.zremrangebyrank('viewed:' + token, 0, -26)  # 最小的到第26个大的删除掉，剩下最大的25个
...
# B Rescale all counts to be 1/2 of what they were before 这里看不大懂
...
class TestCh02(unittest.TestCase):
    def setUp(self):
        import redis
        # host_r = '192.168.1.104'
        host_r = '138.128.194.6'
        port_r = 6379
        db_r = 15
    ...
    def test_login_cookies_my(self):
        conn = self.conn
        global LIMIT, QUIT
        token = str(uuid.uuid4())
        count = 0
        for i in range(0, 30):
            update_token(conn, token, 'username', 'itemX' + str(i))
            count += 1
        print count
        print conn.keys("viewed:?*")
        if USE_THIRTY_DAYS:
            print conn.lrange("viewed:" + token, 0, -1)
            print conn.llen("viewed:" + token)
            self.assertEqual(conn.llen("viewed:" + token), 25)
        else:
            print conn.zrange("viewed:" + token, 0, -1)
            print conn.zcard("viewed:" + token)
            self.assertEqual(conn.zcard("viewed:" + token), 25)
    ...
    def test_cache_request(self):
        ...
        self.assertFalse(can_cache(conn, 'http://test.com/'))  # no item
        self.assertFalse(can_cache(conn, 'http://test.com/?item=itemX&_=1234536'))  # isdynamic
