...
# A Should be run with the location of the GeoLite2-City-Locations-en.csv file
...
def import_cities_to_redis(conn, filename):
    ...
    for count, row in enumerate(csv.reader(open(filename, 'rb'))):
        ...
        country = row[5]
        region = row[7] +row[9] # subdivision_1_name +subdivision_2_name
        city = row[10]
...
class TestCh05(unittest.TestCase):
    ...
    def test_ip_lookup(self):
        ...
        try:
            open('GeoLite2-City-Blocks-IPv4.csv', 'rb')  # GeoLiteCity-Blocks.csv
            open('GeoLite2-City-Locations-en.csv', 'rb')  # GeoLite2-City-Locations-en.csv
        except:
            print "********"
            ...
            print "GeoLite2-City-Locations-en.csv"
            print "********"
            return

        print "Importing IP addresses to Redis... (this may take a while)"
        import_ips_to_redis(conn, 'GeoLite2-City-Blocks-IPv4.csv')
        ...
        import_cities_to_redis(conn, 'GeoLite2-City-Locations-en.csv')
